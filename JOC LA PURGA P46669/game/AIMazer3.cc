...
struct PLAYER_NAME : public Player {
    ...
    void reset() {
        ...
        dist_bazookas = dist_pistoles = dist_guerrers_enemics = dist_enemics_guanyo =
    ...
    /*  Retorna la distancia on es moura
        Canvia direccio a la direccio on és més optim moure's
        Ferit i dia --> Menjar/Barricades   (el més pròxim)
        Ferit, nit
            Enemic proper, Menjar/Barricades lluny --> Fugir
            Diners/Menjar                   (el més pròxim)
        Altre --> Diners */
    int direccio_optima(const Citizen& constructor, Dir& direccio) {
        Pos p = constructor.pos;
        direccio = dir_mes_propera(p, dist_diners);
        int dist = dist_diners[(p+direccio).i][(p+direccio).j];
        if (constructor.life < builder_ini_life()/2 and dist_diners[p.i][p.j] > dist_menjar[p.i][p.j]) {
            direccio = dir_mes_propera(p, dist_menjar);
            dist = dist_menjar[(p+direccio).i][(p+direccio).j];
        }
        return dist;
    }
    ...
    void possibles_atacs(const WeaponType& arma) {
        ...
        buscar_mapa(pos_enemics_guanyo, dist_enemics_guanyo);
    }
    ...
    Dir buscar_arma(const Pos& p, const WeaponType& arma_actual) {
        ...
        if (arma_actual == Gun and pos_bazookas.size() == 0) {
            dir_arma = dir_mes_propera(p, dist_enemics_guanyo);
        }
        else if (dist_bazookas[p.i][p.j] > dist_pistoles[p.i][p.j] and dist_pistoles[p.i][p.j] < 5) {
            dir_arma = dir_mes_propera(p, dist_pistoles);
        }
        prio = dist_enemics_guanyo[(p+dir_arma).i][(p+dir_arma).j];
        return dir_arma;
    }
    ...
    void guerrers() {
        for (int id : meus_guerrers) {
            ...
            if (is_day()) {
                ...
                else direccio = dir_mes_propera(p, dist_enemics_guanyo);
            }
            else {
    ...
    void constructors() {
        for (int id : meus_constructors) {
            ...
            Dir direccio = Up;
            prio = direccio_optima(constructor, direccio);
            accions.push(Accio(id, direccio, prio));
        }
    }
