...
struct PLAYER_NAME : public Player {
    ...
    void possibles_atacs(WeaponType arma, int vida) {
        ...
        for (Pos p : pos_guerrers_enemics) {
            ...
            else if (arma_enemic == arma and enemic.life+life_lost_in_attack() < vida) {
                pos_enemics_guanyo.push_back(p);
            }
        }
    ...
    bool enemic_aprop_arma (int id, const Pos pos_obj) {
        if (es_arma(pos_obj)) {
            Pos p = citizen(id).pos;
            int dist = dist_armes[p.i][p.j].first;
            WeaponType arma_obj = cell(pos_obj).weapon;
            ...
            for (Pos pos_ene : pos_guerrers_enemics) {    
                ...
                if (pos_obj == info_ene.second and dist > info_ene.first and 
                    strongestWeapon(arma_ene, arma_obj) == arma_obj and arma_ene != arma_obj) {
                    return false;
                }
            }
        }
    ...
    void fer_accions() {
        while(!accions.empty()) {
            ...
            if (objectius_agafats.find(p) == objectius_agafats.end() or es_enemic(p) or enemic_aprop_arma(id, p)) {
                move(id, a.dir);
